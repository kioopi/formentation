...
Alongside the `FormData` view, this adapter supplies a `Formentation.Phoenix.StateView` implementation for the Ash form's own submission, issue-visibility, and global-error semantics, so `Formentation.Phoenix.Render.Preparation` renders it unchanged — see [[18-decisions#D-027 — Projection reads semantic state through a StateView protocol|D-027]].
