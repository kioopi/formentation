...
✅ Done (2026-08-11) — supplementary Wave A refactor: the template/occurrence
boundary is explicit. Static nodes and descriptors use `TemplatePath`, runtime
bindings use `InstancePath` through `Formentation.Occurrence`, and render
preparation derives identity from its traversal cursor. See
[[18-decisions#D-050 — Occurrence is the runtime binding of a template node|D-050]],
[[definition-and-node|Definition and Node]], and [[rendering|Rendering]].

✅ Done (2026-08-12) — supplementary Wave B refactor: the `Formentation.Form`
runtime pipeline is decomposed into internal modules. Transition decoding
(`Form.Decoder`), candidate materialization (`Form.Materializer`, owning the
D-026 presence rule), and blocker classification (`Form.Submission`) are
extracted behavior-byte-identical, each with a thin contract test file;
`Form` keeps the struct, public API, transition orchestration, validation
dispatch, and display values. See
...
✅ Decided (2026-08-14) — Milestone B's first blocking decision,
[[phase-1-milestone-b-collections#MB-D1 — Collection semantic model|MB-D1]]
(collection semantic model), is resolved as
[[18-decisions#D-053 — Collections are a dedicated semantic node owning one item template|D-053]]:
a dedicated `Semantic.Collection` struct owning one anonymous item template at
`:item`; `required?` and `:min_items`/`:max_items` cardinality as independent
...
✅ Implemented (2026-08-14) — the **MB-S1 static collection definition slice**
([issue #52](https://github.com/kioopi/formentation/issues/52)): a
`Semantic.Collection` node kind with an anonymous `:item` template, a
`Presentation.Collection` layout descriptor, finalizer invariants
(shape/locality raises + `:invalid_cardinality` diagnostics per
[[18-decisions#D-052 — A `%Definition{}` is final|D-052]]),
`Info.item_template/2` and `Info.constraints/2`, the shared
`Context.enter_item/1` adapter seam, both source compilations with every
[[18-decisions#D-054 — Collection source vocabularies and the degradation table|D-054]]
degradation row tested, paired scalar/object differential fixtures, and
`format_version` 4
([[18-decisions#D-055 — `format_version` bumps on any stored-representation vocabulary change|D-055]]).
Runtime stays deliberately gated: `Form.new/3` rejects collection
definitions until MB-T4/T5 land concrete occurrences.
