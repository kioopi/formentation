...
3. [[source-adapters|Source adapters]] — how adapters are selected (built-in selectors and module extension), the `Formentation.Definition.Source` behaviour and its two implementations, and the differential property that keeps them equivalent. **Written.**
