...
Implementation began on 2026-07-21. As of 2026-08-09, [[phase-1-walking-skeleton|Phase 1]] Milestone A runs end to end and is aligned with the north-star architecture ([[phase-1-north-star-alignment|the alignment gate]], released as `v0.2.0`): both source adapters compile to a `Definition` holding separate semantic and presentation structures, `Formentation.Form` holds runtime state and decides submissions, `Phoenix.HTML.FormData` projects that state, and render preparation plus the reference UI render it as accessible HTML — [[17-end-to-end-example|the end-to-end example]] is pinned as a reviewed snapshot, and the LiveView lifecycle ships with a runnable demo and a browser-real suite. The remaining Phase 1 work is collections (Milestone B). [[Development]] carries the phase-status table; [[Techdocs]] documents what is built and [[Userguide]] how to use it.
