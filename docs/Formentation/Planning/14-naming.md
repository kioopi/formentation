...
`Formentation.Phoenix.Render.Preparation`, `Render.Plan`, `Render.Node`, and
`Formentation.Phoenix.UI.Reference` are current Phase 1 implementation names.
They do not define the future public vocabulary. The former `Projector` and
theme-derived component names were superseded first by D-041 and then by the
D-047 module-tree restructure. See
