...
- ~~What is the minimal state-view interface required beyond `%Phoenix.HTML.Form{}`?~~ Answered 2026-07-22: the per-field read model is `Formentation.Form.FieldState` (transport, operation, usage, issues, derived display value) plus `Form.show_issues?/2` for visibility; collection and branch accessors still wait for Milestone B and Phase 4 — step-4 spec `docs/superpowers/specs/2026-07-22-phase1-step4-state-and-codecs-design.md`.
...
- Should the map source eventually provide instance validation for parity with the JSON Schema adapter? Step 4 gave `Formentation.Definition` an opaque `validator` slot that only the JSON Schema adapter fills; map-source forms silently skip schema validation (decode semantics are unaffected). Options when a use case arrives: compile the map declaration to an equivalent JSON Schema, or accept validation as adapter-specific.
