...
- ~~Number inputs render `type="number"`; browsers may refuse to display non-numeric raw text (e.g. `"51o2"`) even though the attribute carries it.~~ Answered 2026-07-24: confirmed against a real browser (Chrome) — it fails raw-input preservation two ways at once, blocking non-numeric keystrokes outright and, when invalid text is force-injected anyway, sanitizing it away on the round-trip patch. The number widget now ships as `type="text"` instead of `type="number"`; its hint is `numeric` for integers and `decimal` for general numbers (commit "Fall back number inputs to text with numeric inputmode", narrowed by [[18-decisions#D-038 — Semantic value type and abstract widget are orthogonal prepared facts|D-038]]) — [[18-decisions#D-021 — LiveView integration is wrappers plus a demo, not framework machinery|D-021]].
