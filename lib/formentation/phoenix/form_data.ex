defimpl Phoenix.HTML.FormData, for: Formentation.Form do
  ...
  # once an action exists — per-field visibility belongs to used_input?/1
  # (core components) and Form.show_issues?/2 (themes), never to this
  # projection. Object-level and root issues stay out of Phoenix's
  # per-field convention; themes read them via Form.issues/2.
  ...
  # object, for example) is object-level and stays out — themes read it
  # via Form.issues/2 under the show_issues?/2 submit gate.
