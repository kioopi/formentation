...
`Formentation.compile/2` compiles plain Elixir data (`Formentation.Source.Map`) or a JSON Schema document — draft 2020-12, validated offline against the metaschema, with a UI-hints channel for ordering, presentation groups, and widget/help overrides (`Formentation.JSONSchema`) — into a `Formentation.Definition` queried through `Formentation.Info`, with origin provenance on every resolved value, structured diagnostics, and depth/node guards. A differential test asserts both sources answer `Info` queries identically apart from origins. `Formentation.Form` holds runtime state with pure replace transitions, typed decode policies, and whole-instance validation deferred while any decode fails. That state implements `Phoenix.HTML.FormData`, and `Formentation.Phoenix.fields/1` renders it as accessible HTML through a reference theme — composing inside a form you wrote yourself. `Formentation.Form.validate/2` and `Formentation.Form.submit/2` are the `phx-change`/`phx-submit` entry points for LiveView — thin wrappers over the same transitions, nothing framework-shaped. Run `mix demo` to see it live: a pump-inspection form at `/`, a nested-object form at `/nested`, both backed by the same LiveViews `test/formentation_demo/` exercises.
...
mix ci      # full gate: compile with warnings-as-errors, format check, tests,
            # credo --strict, dialyzer, duplication and architecture checks
